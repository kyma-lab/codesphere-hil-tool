...
# copied version from the shared folder, this IS NOT UP TO DATE, DO NOT MAKE CHANGES HERE, instead change the version in the shared folder


def validateIOB_Testset(data):
    # for testsets, we only check the format (1 element per line, which is the text)
    #! check if this can handle empty lines / newlines (currently do not occur in input)
    lines = data.split("\n")
    violation_counter = 0
...
def validateIOB(data, count, verbose, tab_separated):
    # expects IOB document as single string
    lines = data.split("\n")
    #print(f"Split file into {len(lines)} tokens")
    violation_counter = 0
    previous_label = "O"
    classes = []
