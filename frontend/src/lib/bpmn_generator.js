...
function add_event(root, diagram, processid, eventlabel, x, y, textannotation = "") {
    ...
    for (let i = 0; i < root.children.length; i++) {
        if (root.children[i].values.id == processid) {
            ...
            log('BPMN_generator', 'Added event ' + eventid + ' to process ' + processid);
        }
    }
        ...
        if (root.children[i].values.id == processid) {
            ...
            root.children[i].children.push(
                create_task(
                    activity_id,
                    activity_label));
            log('BPMN_generator', 'Added task ' + activity_id + ' to process ' + processid);
        }
    }
    ...
    if (textannotation != "") {
        ...
        for (let i = 0; i < root.children.length; i++) {
            if (root.children[i].values.id == processid) {
                ...
                log('BPMN_generator', 'Added annotation ' + annotationid + ' to process ' + processid);
            }
        }
...
function add_element(root, type, diagram, processid, labeltext, x_coord, y_coord, annotationtext) {
    if (type == 'Standard') {
        ...
        log('BPMN_generator', 'Empty task found')
    } else {
        log('BPMN_generator', 'Unknown task type found')
    }
}
